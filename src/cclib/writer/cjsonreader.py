...
class CJSON:
    ...
    def generate_properties(self, input_dict):
        ...
        self.fill_cjson_dict(self.datatype['properties'], input_dict, attr_list)

        if "orbitals" in input_dict:
            orbital_attr = ['homos', 'moenergies', 'aooverlaps', 'mosyms', 'mocoeffs']
            self.fill_cjson_dict(self.datatype['properties']['orbitals'], input_dict["orbitals"], orbital_attr)

        if "energy" in input_dict:
            self.datatype['properties']['energy']['alpha']['homo'] = input_dict['energy']['alpha']['homo']
            self.datatype['properties']['energy']['alpha']['gap'] = input_dict['energy']['alpha']['gap']
            self.datatype['properties']['energy']['beta']['homo'] = input_dict['energy']['beta']['homo']
            self.datatype['properties']['energy']['beta']['gap'] = input_dict['energy']['beta']['gap']
            self.datatype['properties']['energy']['total'] = input_dict['energy']['total']
            energy_attr = ['freeenergy', 'mpenergies', 'ccenergies']
            self.fill_cjson_dict(self.datatype['properties']['energy'], input_dict['energy'], energy_attr)
        ...
        if 'elements' in input_dict:
            ...
            for name in attr_list:
                if name in input_dict['elements']:
                    self.datatype['atoms']['elements'][name] = input_dict['elements'][name]

        if 'coords' in input_dict:
            self.datatype['atoms']['coords']['3d'] = input_dict['coords']['3d']

        if 'orbitals' in input_dict:
            orbital_list = ['aonames', 'atombasis']
            self.fill_cjson_dict(self.datatype['atoms']['orbitals'], input_dict['orbitals'], orbital_list)

        key_list = ['coreelectrons', 'atommasses', 'atomspins']
        self.fill_cjson_dict(self.datatype['atoms'], input_dict, key_list)

    def generate_optimization(self, input_dict):
        attr_list = ['optdone', 'optstatus', 'geotargets', 'geovalues', 'nbasis', 'nmo']
        self.fill_cjson_dict(self.datatype['optimization'], input_dict, attr_list)

        if 'scf' in input_dict:
            scf_list = ['scfenergies', 'scftargets', 'scfvalues']
            self.fill_cjson_dict(self.datatype['optimization']['scf'], input_dict['scf'], scf_list)

        if 'scan' in input_dict:
            attr_list = ['scancoords', 'scanenergies', 'scannames', 'scanparm']
            self.fill_cjson_dict(self.datatype['optimization']['scan'], input_dict['scan'], attr_list)

    def generate_vibrations(self, input_dict):
        attr_list = ['vibanharms', 'vibfreqs', 'vibsyms', 'hessian', 'vibdisps']
        self.fill_cjson_dict(self.datatype['vibrations'], input_dict, attr_list)

        if 'intensities' in input_dict:
            intensities_list = ['vibirs', 'vibramans']
            self.fill_cjson_dict(self.datatype['vibrations']['intensities'], input_dict['intensities'],
                                 intensities_list)

    def generate_bonds(self, input_dict):
        if 'connections' in input_dict:
            self.datatype['bonds']['connections']['index'] = input_dict['connections']['index']
        if 'order' in input_dict:
            self.datatype['bonds']['order'] = input_dict['order']

    def generate_transitions(self, input_dict):
        attr_list = ['etenergies', 'etoscs', 'etrotats', 'etsecs', 'etsyms']
        self.fill_cjson_dict(self.datatype['transitions'], input_dict, attr_list)

    def generate_fragments(self, input_dict):
        attr_list = ['fragnames', 'frags', 'fonames', 'fooverlaps']
        self.fill_cjson_dict(self.datatype['fragments'], input_dict, attr_list)
