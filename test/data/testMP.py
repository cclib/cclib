...
class GenericMP2Test:
    ...
    def testsizeandshape(self, data) -> None:
        ...
        assert data._ccCollection._parsed_data[0].mpenergies.shape == (len(data._ccCollection._parsed_data[0].scfenergies), self.level - 1)

    def testsign(self, data) -> None:
        """Are the Moller-Plesset corrections negative?"""
        if self.level == 2:
            corrections = data._ccCollection._parsed_data[0].mpenergies[:, 0] - data._ccCollection._parsed_data[0].scfenergies
        else:
            corrections = data._ccCollection._parsed_data[0].mpenergies[:, self.level - 2] - data._ccCollection._parsed_data[0].mpenergies[:, self.level - 3]
