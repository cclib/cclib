...
class GenericTDTest(unittest.TestCase):
    ...
    def testsecs_transition(self):
        ...
        self.assertTrue(
            t[0][1][0] == self.data.homos[0] or t[0][2][0] == self.data.homos[0] + 1, t[0]
        )
...
class GaussianTDDFTTest(GenericTDTest):
    ...
    def testetveldipsshape(self):
        ...
        self.assertEqual(numpy.shape(self.data.etveldips), (self.number, 3))
...
class OrcaTDDFTTest(GenericTDTest):
    ...
    def testoscs(self):
        """Is the maximum of etoscs in the right range?"""
        self.assertEqual(len(self.data.etoscs), self.number)
        self.assertAlmostEqual(max(self.data.etoscs), 0.09, delta=0.01)
