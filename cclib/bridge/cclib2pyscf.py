...
def makecclib(method, etmethod = None) -> ccData:
    ...
    if etmethod:
        ...
        attributes["etoscs"] = etmethod.oscillator_strength(gauge = "length") # or do we want velocity?
        # etmethod.analyse() prints real symmetries, so they must be available somehwere...
        attributes["etsyms"] = ["Singlet" if etmethod.singlet else "Triplet" for i in range(0, len(attributes["atomnos"])) ]
