...
class CFOUR(logfileparser.Logfile):
    ...
    def extract(self, inputfile, line):
        ...
        if "Coordinates (in bohr)" in line:
            ...
            coord_block = numpy.array(
                self._parse_block(
                    inputfile, self.natom, str, "parsing coordinate blocks"
                )
            )
        ...
        # check to see if atom symmetry is accounted for:
        ang_mom_map = {"S": "S", "X": "P", "XX": "D", "F300": "F", "G400": "G"}
        if "There are" in line and "basis functions." in line:
            self.nbasis = int(line.split()[2])

        if "TYPES OF ATOMS" in line:
            self.sym_atom_vec = None
            sym_atom_types = int(line.split()[0])
            if sym_atom_types != self.natom:
                print("we are herrrreee even if we shouldne be")
                while "INTERNUCLEAR DISTANCES (A) :" not in line:
                    line = next(inputfile)
                if "INTERNUCLEAR DISTANCES (A) :" in line:
                    self.skip_lines(inputfile, ["blank"])
                    line = next(inputfile)
                    self.sym_atom_vec = [int(line.split()[3].strip("#"))]
                    self.skip_lines(inputfile, ["blank"])
                    for i in range(self.natom - 1):
                        line = next(inputfile)
                        anum = int(line.split()[1].strip("#"))
                        # store the type of each atom for the ordering in input coordinates.
                        self.sym_atom_vec.append(anum)

        if "GAUSSIAN BASIS INFORMATION" in line:
            self.skip_lines(inputfile, ["blank", "column_titles", "blank"])
            line = next(inputfile)
            self.sym_unique_gbasis = [[]]
            ...
            while not basis_done:
                ...
                for i in range(len(overall_basis)):
                    self.sym_unique_gbasis[-1].append(
                        (ang_mom_map[ang_mom], overall_basis[i])
                    )
                if basis_done:
                    break
                split_line = line.split()  # example line:  O #1  1    S
                ...
                if new_atom_num != atom_num:
                    self.sym_unique_gbasis.append([])
                    atom_num = new_atom_num
                line = next(inputfile)
            # If the molecule has symmetry, this should expand the gbasis object to have an entry for every atom, not just the symmetry unique ones.
            self.gbasis = []
            if self.sym_atom_vec is not None:
                for a in self.sym_atom_vec:
                    self.gbasis.append(self.sym_unique_gbasis[a])
            else:
                self.gbasis = self.sym_unique_gbasis
