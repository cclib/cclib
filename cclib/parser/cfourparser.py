            ...
            while True:
                ...
                if "#" in line:
                    ...
                    if len(split_line[3]) == 1:
                        ...
                        if (
                            (int(split_line[3][-3]) > 2)
                            and (int(split_line[3][-2]) == 0)
                            and (int(split_line[3][-1]) == 0)
                        ):
                ...
                if should_parse and ("#" not in line):
                    ...
                    for i in range(line_length - 2):
                        if not float(split_line[2 + i]) == 0.0:
                            temp_basis_info[i].append(
                                (float(split_line[1]), float(split_line[2 + i]))
                            )
        ...
        if "Coordinates used in calculation (QCOMP)" in line:
            ...
            while "----------------------------------------------------------------" not in line:
                ...
                temp_atomcoords.append(
                    [
                        utils.convertor(utils.float(line.strip().split()[2]), "bohr", "Angstrom"),
                        utils.convertor(utils.float(line.strip().split()[3]), "bohr", "Angstrom"),
                        utils.convertor(utils.float(line.strip().split()[4]), "bohr", "Angstrom"),
                    ]
                )
                line = next(inputfile)
            if self.first_coord_block:
                                ...
                                if len(split_line[1]) >= 3:
                                    if split_line[1][1] == "X":
                                        try:
                                            ...
                                            pass
                                aonames.append(
                                    f"{self.atomic_symbols[(int(split_line[0]) - sub_amount) - 1]}{int(split_line[0]) - sub_amount}_{subshell_number}{split_line[1]}"
                                )
                                atombasis[-1].append(basis_index)
                                basis_index += 1
                            for i in range(len_split_line - 2):
                                self.temp_alpha_mocoeffs[i - (len_split_line - 2)].append(
                                    utils.float(split_line[i + 2])
                                )
                            line = next(inputfile)
                        if self.parse_aonames:
