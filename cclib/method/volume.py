...
class Volume(object):
    ...
    def writeascube(self, filename):
        ...
        origin = [convert((x + y) / 2) for x, y in zip(self.origin, self.topcorner)]
...
def read_from_cube(filepath):
    ...
    vol = Volume(
        (
            convertor(originx - 0.5 * ngridx * spacingx, "bohr", "Angstrom"),
            convertor(originy - 0.5 * ngridy * spacingy, "bohr", "Angstrom"),
            convertor(originz - 0.5 * ngridz * spacingz, "bohr", "Angstrom"),
        ),
        (
            convertor(originx + 0.5 * ngridx * spacingx - 0.5 * spacingx, "bohr", "Angstrom"),
            convertor(originy + 0.5 * ngridy * spacingy - 0.5 * spacingy, "bohr", "Angstrom"),
            convertor(originz + 0.5 * ngridz * spacingz - 0.5 * spacingz, "bohr", "Angstrom"),
        ),
        (
