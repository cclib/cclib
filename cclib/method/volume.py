...
def read_from_cube(filepath):
    ...
    # Cube files are written in bohrs
    def convert(x):
        return convertor(x, "bohr", "Angstrom")
    ...
    vol = Volume((convert(originx - 0.5 * ngridx * spacingx), convert(originy - 0.5 * ngridy * spacingy), convert(originz - 0.5 * ngridz * spacingz)), (convert(originx + 0.5 * ngridx * spacingx), convert(originy + 0.5 * ngridy * spacingy), convert(originz + 0.5 * ngridz * spacingz)), (convert(spacingx), convert(spacingy), convert(spacingz)))
