...
class Bader(Method):
    ...
    def calculate(self, indices=None, fupdate=0.05):
        ...
        if len(self.data.mocoeffs) == 1:
            self.chgdensity = electrondensity_spin(self.data, self.volume, [self.data.mocoeffs[0][:self.data.homos[0]]])
            self.chgdensity.data *= 2
        else:
            self.chgdensity = electrondensity_spin(self.data, self.volume, [self.data.mocoeffs[0][:self.data.homos[0]], self.data.mocoeffs[1][:self.data.homos[1]]])
            ...
            for ywalker in range(1, len(self.chgdensity.data[0])-1):
                for zwalker in range(1, len(self.chgdensity.data[0][0])-1):
                    ...
                    if self.fragresults[xindex, yindex, zindex] != 0:
                        ...
                        while (loopcondition):
                            tmp = self.chgdensity.data[xindex-1:xindex+2, yindex-1:yindex+2, zindex-1:zindex+2] - self.chgdensity.data[xindex, yindex, zindex]
                            grad = tmp / [[[1.73205, 1.41421, 1.73205], [1.41421, 1, 1.41421], [1.73205, 1.41421, 1.73205]], [[1.41421, 1, 1.41421], [1, 1, 1], [1.41421, 1, 1.41421]], [[1.73205, 1.41421, 1.73205], [1.41421, 1, 1.41421], [1.73205, 1.41421, 1.73205]]]
                            ...
                            if self.fragresults[xindex+nextDirection[0], yindex+nextDirection[1], zindex+nextDirection[2]] != 0:
                                # Path arrived at a point that has already been assigned with an index
                                baderIndex = self.fragresults[xindex+nextDirection[0], yindex+nextDirection[1], zindex+nextDirection[2]]
                            ...
                            elif xindex + nextDirection[0] == 0 or xindex + nextDirection[0] == (len(self.chgdensity.data)-1) or yindex + nextDirection[1] == 0 or yindex + nextDirection[1] == (len(self.chgdensity.data[0])-1) or zindex + nextDirection[2] == 0 or zindex + nextDirection[2] == (len(self.chgdensity.data[0][0])-1):
        ...
        for pos in range(len(self.data.atomcoords[-1])):
            gridpt = numpy.round((self.data.atomcoords[-1][pos] - self.volume.origin)/self.volume.spacing)
        ...
        assert 0 not in self.matches, "Failed to assign Bader regions to atoms. Try with a finer grid. Content of Bader area matches: {}".format(matches)
        assert len(numpy.unique(self.matches) != len(self.data.atomnos)), "Failed to assign unique Bader regions to each atom. Try with a finer grid."
        
        # Finally integrate the assigned Bader areas
        self.logger.info("Creating fragcharges: array[1]")
        boxvol = (self.volume.spacing[0] * self.volume.spacing[1] * self.volume.spacing[2] * convertor(1, "Angstrom", "bohr") ** 3)
